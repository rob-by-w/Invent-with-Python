...
CORRECT_TEXT = ['Correct!', 'That is right.',
                "You're right.", 'You got it.', 'Righto!']
INCORRECT_TEXT = ['Incorrect!', "Nope, that isn't it.",
                  'Nope.', 'Not quite.', 'You missed it.']
...
for questionNum, questionAnswer in enumerate(QUESTIONS):
    os.system('cls' if os.name == 'nt' else 'clear')
    print(f'Question: {questionNum+1}')
    print(f'Score: {score}/{len(QUESTIONS)}')
    print(f"QUESTION: {questionAnswer['question']}")
    userAnswer = input('  ANSWER: ').lower()

    if userAnswer == 'quit':
        print('Thanks for playing!')
        sys.exit()
    if any([keyword in userAnswer for keyword in questionAnswer['accept']]):
        print(random.choice(CORRECT_TEXT), end=' ')
        score += 1
    else:
        print(random.choice(INCORRECT_TEXT), end=' ')
    print(f"The answer is: {questionAnswer['answer']}")
